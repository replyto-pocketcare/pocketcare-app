...
## Key files
`app/loans/page.tsx` (list + AddLoan), `app/loans/[id]/page.tsx` (detail, fixed schedule + variable EMI list, mark-paid dialog, edit), `@sanvya/finance` (`emiFromPrincipal`, `amortizationSchedule`, `emiDueDate`, `isDuePassed`, `effectivePaidEmis`). Migrations `0034` (due-day/auto-mark) + `0036` (rate_type/emi_amounts).
