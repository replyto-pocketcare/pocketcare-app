...
- iOS: `Data/Sources/Data/UpiRepository.swift` (extended -- already existed with the own-handle helpers; `fetchCounterpartyHandle` + a `SupabaseClient` dependency added this pass), `Data/Sources/Data/SplitsRepository.swift` (extended: `getGroup(groupId:)`), `App/ViewModels/SplitsViewModel.swift` (rewritten), `App/SplitsView.swift` (rewritten), `App/CreateGroupView.swift` (rewritten -- was a dead mockup whose Create button just called `dismiss()`), `App/GroupDetailView.swift` (new), `App/ViewModels/GroupDetailViewModel.swift` (new), `App/PayViaUpiSheet.swift` (small addition: an `onPaid: (String) -> Void = { _ in }` callback fired before dismiss on "I've paid" -- the real file had no callback at all, so Splits' settle-up flow could not have recorded the pending settlement without this; everything else in the file is unchanged), `Data/Sources/Data/DI/DataModule.swift` (extended: `upiRepository` Factory).
