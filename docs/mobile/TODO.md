...
```
Last session: 2026-08-07 — Receipt Scan capture (task #62, P3.22), both platforms, 2nd piece of the
               3-part request from task #30's session (Splits done / Receipt Scan done / BLE
               split-detection next). Built real camera capture + on-device OCR + full-parity review
               screen on top of the already-existing-but-unwired ReceiptsRepository/domain/receipts
               scaffolding: Android got CameraX+ML Kit from zero (new Gradle deps, CAMERA permission);
               iOS's old ReceiptScanView.swift (hardcoded "Olive Garden" fixture, no camera/OCR/repo
               wiring, not in any navigation) was deleted entirely and replaced with
               UIImagePickerController+Vision. Added a Dashboard speed-dial FAB on both platforms (real
               port of web's AddSpeedDial) as the entry point -- neither mobile Dashboard had ANY
               quick-add control before this. Documented scope cuts (see receipt-scan.md): camera-only
               (no PDF/upload), no AI escalation, text-only OCR parsing (not word-box groupIntoLines),
               "Split this bill" shown disabled -- deferred to P3.23 on purpose, since that feature's
               participant-selection UI will be BLE-driven, not manual per-item taps, and building the
               web-parity itemized screen now risked getting thrown away. Found + fixed mid-session: a
               real pre-existing bug, not mine -- GroupDetailView.swift/GroupDetailViewModel.swift
               (built last session for task #30) were never added to project.pbxproj, so the iOS app
               would not have compiled; fixed by hand-editing the pbxproj (PBXBuildFile/PBXFileReference/
               PBXGroup/PBXSourcesBuildPhase, all 4 sections, verified 4 occurrences per new file).
               New receipt-scan.md spec, receipt-scanning.md feature doc got a "Mobile status" section.
               NOT yet build-verified by a real compiler. NEXT: the new BLE "enable detection for split"
               feature (P3.23, Akhilesh chose true Bluetooth proximity over a join-link/QR-code --
               biggest unknown, no BLE code exists in either app yet -- needs a design pass first, its
               own participant-selection UI will finally unblock the itemized split-write path this
               session deliberately didn't build), then Auto-categorization (P3.3e, was assumed
               edge-function-gated, actually fully local/client-side on web, no entitlement gate).
Previous session: 2026-08-07 — Splits screen (task #30), both platforms. Real SplitsRepository already
               existed (P2.5) on both platforms; that pass built the missing UI + fixed 2 real bugs:
               `"Friend" // Placeholder` names (never joined `connections`) on BOTH platforms, and iOS's
               settle-up sheet hardcoded `amountMinor: 120000` (every settle-up showed ₹1200 regardless
               of real balance). Built: friends/groups hub, group detail (equal-split-only add-expense --
               percent/exact/itemized deferred), create-group, real UPI settle-up (new
               ...
               Deliberately deferred: invite/share-link flow (P3.11, corrected to TODO that session --
               ...
               AUDIT_HISTORY.md's 2026-08-07 entry.
...
| P3.13a / P3.13b | S4: Receipt scan — CameraX+ML Kit / AVFoundation+Vision **with word bounding boxes** → ported line-rebuild + reconciliation gate UI (review must not save until Σ lines == total) | [H] | P3.3 | SUPERSEDED by P3.22 (2026-08-07, task #62) — capture/OCR/review shipped there, but via the **text-only** parse path (`parseReceiptText`), not this row's "with word bounding boxes" (`groupIntoLines`) scope — see receipt-scan.md scope note #3 for why. `groupIntoLines`/`parseReceipt` stay ported and tested for a future pass that wires up per-word boxes from ML Kit/Vision. |
...
| P3.22a / P3.22b | Receipt scan capture, for real — wire the already-real-but-unused `ReceiptsRepository`/`domain/receipts/*` (parse/reconcile/allocate, P2.6-era) into an actual camera capture + OCR screen on both platforms, replacing Android's total absence and iOS's disconnected `ReceiptScanView.swift` mockup. Feeds P3.23 below. | [H] | P3.13 | DONE (2026-08-07, task #62) — see `docs/mobile/screen-specs/receipt-scan.md` for the full scope. Shipped: camera capture (CameraX / `UIImagePickerController`+Vision), on-device OCR (ML Kit / `VNRecognizeTextRequest`, text-only path — not the word-box `groupIntoLines` path), full-parity review screen (editable fields/lines, reconciliation banner + both one-tap fixes, save-as-transaction), a Dashboard speed-dial FAB (neither platform had ANY quick-add control before this). `ReceiptScanView.swift`'s old hardcoded fixture deleted entirely. Documented cuts: no PDF/gallery upload, no AI escalation, "Split this bill" shown disabled (deferred to P3.23 — building the manual itemized-assignment screen now risked being thrown away once BLE-driven participant selection lands), no category auto-suggest (pairs with P3.3e). Also fixed a real pre-existing gap found mid-session: `GroupDetailView.swift`/`GroupDetailViewModel.swift` (built for task #30) were never added to `project.pbxproj` — the iOS app would not have compiled. NOT yet build-verified by a real compiler (no Xcode/Gradle in this sandbox). |
