...
| Splits tile reactivity | Android's repository combines two watches, so the tile updates live | iOS re-reads a one-shot `friendBalances` whenever the connections watch fires | Not a choice made for this tile. iOS's `AsyncThrowingStream` has **no `combine` operator**, so every derived/composed read in `SplitsRepository` and `LedgerRepository` is a one-shot snapshot there and a real `Flow.combine` on Android. Closing it properly means building a `combineLatest` for `AsyncThrowingStream` — worth doing, and too load-bearing to bolt onto a tile. **`SplitsRepository.swift`'s own REACTIVITY NOTE claims the Kotlin mirror is one-shot too; it is not, and the note is stale** |
