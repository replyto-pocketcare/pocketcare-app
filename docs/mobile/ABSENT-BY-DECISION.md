...
| Assistant composer | The **mic button** (`speech.ts` + `MicButton.tsx`) | Web uses the Web Speech API. Native equivalents exist on both platforms but carry a runtime microphone permission, an on-device/network model choice and a partial-results UI — its own port with its own decisions, not a button to bolt on. The composer is otherwise complete |
