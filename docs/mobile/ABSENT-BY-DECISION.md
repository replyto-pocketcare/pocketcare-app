...
| Assistant, out of quota | The **credit-pack purchase** (three Razorpay top-ups) **and the sentence that advertises it** | Re-examined 2026-08-29 end to end and the answer got firmer. `buyCredits()` calls the `razorpay-credits` edge function for an order id, then `loadRazorpay()` injects `checkout.razorpay.com/v1/checkout.js` into the **page** and opens the web Checkout modal. A phone has no page to inject it into. The native Razorpay SDKs are not the fix either: AI credits are digital content consumed in-app, so App Store 3.1.1 and Google Play's payments policy both require StoreKit / Play Billing, and `razorpay-webhook` is the only thing that grants credits server-side — there is no receipt-verification path for either store. So this is Play Billing + StoreKit + a new server grant path: a project, not a control.<br><br>**`outPaidRest` is omitted with the buttons, on purpose.** Web's paid card is "You've used all your AI prompts for this cycle." (bold) + " Buy a credit top-up to keep going — credits never expire." The second sentence is the caption for the three buttons. Shipping it without them would be this file's own failure mode in prose — copy telling the user to do something the app gives them no way to do. The bold half stands alone and is what both phones render; the landing screen already states the reset date. Both screens' headers say so |
