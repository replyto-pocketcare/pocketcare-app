...
| Dashboard → Spending tile | Web's **`lend` exclusion**, on THAT tile only | The by-category and by-label tiles carry web's `lend` exclusion in SQL (added 2026-08-26). The **Spending this month** tile still does not: it filters transactions in Kotlin/Swift rather than SQL, so it has no subquery to hang the exclusion on, and **reads high for anyone who lends**. Closes when it moves to a grouped query like its two siblings |
...
| Dashboard | **Five of the fourteen tiles** | Built: Recent activity, Spending this month, Upcoming payments, Budgets, Goals, Friends & splits, Spending by category, Spending by label, This month vs last. The other five (Expense trends, Cashflow, Net cashflow trend, Subscriptions, Across currencies) are **absent from the Add-a-widget picker**, not offered-and-broken: `TileId.isBuilt` gates both the picker and the grid, and it is an exhaustive `when` / `switch` with no fallthrough, so a fifteenth tile in web's catalog fails the native build until somebody decides. The charts they need now exist; what is left is `buildTrend`'s bucketing (which belongs in domain, vector-tested, like `packRows`) and the monthly cashflow aggregation |
