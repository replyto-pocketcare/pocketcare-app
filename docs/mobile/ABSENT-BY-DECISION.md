...
| Dashboard → Spending tile | Web's **`lend` exclusion**, on THAT tile only | Every other spending query on the dashboard now carries web's `lend` exclusion — by category, by label, monthly cashflow, daily trend. The **Spending this month** tile still does not: it filters transactions in Kotlin/Swift rather than SQL, so it has no subquery to hang the exclusion on, and **reads high for anyone who lends**. It is now the only one. Closes when it moves to a grouped query like its siblings |
