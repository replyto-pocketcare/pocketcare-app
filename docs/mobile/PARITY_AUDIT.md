...
| `/statements/analyze` | (329) + (419 in `src/statements`) | 🔶 domain only | 🔶 domain only | 🔶 **2026-08-27: the whole pure core is ported and vector-pinned** — `parseStatementCsv`, `summarize`/`byCategory`/`byMonth`/`byDay`/`outliers`/`recurringCandidates`, and `reconcileStatement`, under 61 vectors generated by running web's own modules. The SCREEN is next; PDF text extraction is its own problem (see below) |
