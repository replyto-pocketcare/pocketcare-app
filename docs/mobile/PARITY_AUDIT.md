...
18. **The receipt capture screen's plan card and its PDF errors are not in the
    catalogue.** `apps/web/app/receipts/new/page.tsx` writes `t("premium.title",
    "A Lite or Pro feature")` and two siblings with inline defaults, and
    `scan.ts` throws four error strings — "That PDF has no readable text…",
    "Couldn't read that PDF: …" and the two password ones — as bare English in
    `new ScanError(...)`, where no translator can reach them at all.

    Hindi and Dutch therefore render the whole paywall card and every scan
    failure in English. Added to `packages/core/i18n/src/locales/receipts/` in
    all three locales while porting the screen: `premium.title`,
    `premium.body`, `premium.cta`, `errors.pdfNoText`, `errors.pdfUnreadable`,
    `errors.unsupportedFile`. Web keeps working unchanged and picks the first
...
## Tranche 7 — the shell, and the thing underneath it (2026-08-29)

Four gaps closed, four pollers collapsed into one, and **one finding that is
larger than everything in this workstream put together** — see the next section.

| Closed | Note |
|---|---|
| "Worth a look" suggestions strip | Ranking in Domain under **32 vectors generated by running web's own `@sanvya/suggestions`**, so every threshold is pinned to ground truth rather than transcribed |
| EXPANDED-width KPI strip + embedded Ask Sanvya | Gated on the existing window-size class. The four figures come off the **same monthly fold the hero's sparkline already does**; web re-runs the identical `GROUP BY` in a second component |
| Sync-status strip | Plus `syncNotice()` in Domain under 40 vectors |
| TrialNotice | Countdown banner and one-time welcome, keyed on web's own `sanvya:trial-welcome:<email>` |

**BlockingLoader was deliberately not wired.** There is no web counterpart to
port: web has exactly one loader (`GlobalLoader`, a 24px corner spinner) and it
is already ported as `WriteIndicator` on both platforms. Android's
`BlockingLoader` is an invention with no original, so inventing a call site for
it would be *building a control web does not have*. The parity-correct action is
to delete it.

**Scroll restoration: Android done, iOS deferred with a named fix.** Android now
uses `popUpTo(saveState = true)` + `restoreState = true` — the platform's own
per-route state restoration, which also restores filter chips and half-typed
search because every `remember*State` is saveable. Web hand-rolls
`pc_scroll:<path>` only because a browser has no such thing. **This was actively
broken before**: every bottom-bar tap discarded the destination's state. iOS
cannot do it from a screen: `ContentView`'s `screen(for:)` is a `@ViewBuilder`
switch, so SwiftUI tears the outgoing screen down and there is no offset left to
restore. The fix is one architectural change — a `TabView(selection:)` with
hidden chrome, which retains each screen for free — and it belongs in a pass that
owns `ContentView.swift`.

### The sync status consolidation

There was no shared sync status: `hasSynced` on `SettingsRepository`, `connected`
from `syncSnapshot()`, real connectivity trapped privately inside the shell
(`rememberIsOffline` / `NWPathMonitor`), and **four independent 400ms pollers**
on the same field. Now one `SyncStatusRepository` / `SyncStatusStore` exposing
online · connected · hasSynced · lastSyncedAt · error.

iOS needs **no poll at all** — `SyncStatus.asFlow()` is a real change stream in
the Swift SDK, verified against the vendored checkout. Android keeps one loop
(400ms until first sync, then 2s) and only while subscribed. Connectivity moved
out of the shell into `:data`, which is why `apps/android/data` now has its own
`AndroidManifest.xml` declaring `ACCESS_NETWORK_STATE`.

### Deliberate divergences

**The strip's OFFLINE branch never renders on mobile.** Web says "you are
offline" **twice** — once in the sticky `OfflineBanner` and again, in different
words, in the in-flow strip. On a browser that is redundant; on a phone it is two
of the six visible rows saying the same thing on every screen. The banner is the
better of the two because it is pinned, so the strip keeps only TROUBLE.

**Force Sync is absent from the strip.** Web's button is `disconnect()` then
`connect(connector)` — see the next section for why wiring that here would have
been the worst possible place to discover the problem.

### Caught in review, before commit

**The suggestions strip would have been dead on arrival, silently.** The query
filters `credit_card_details` on `deleted_at`, and that is the one table of the
twelve without that column. SQLite raises `no such column`, the stream throws on
first evaluation, the catch is empty, and the strip renders nothing forever —
the exact failure its own header comment brags about fixing on web. Web has this
bug *too*, on top of its trailing comma, so **web's strip has never rendered for
anybody**. Both are fixed here, which makes the port the first time the ranking
has actually run.

**The trial welcome dialog fired twice, under two different keys.**
`sessionEmail` arrives asynchronously, so the first render read the key for a nil
email, showed the dialog, and marked *that* key on dismissal; when the address
landed the key changed and the same person was welcomed again. Android already
gated on a non-nil email; iOS now does too.

**The docked assistant panel was clipped to a 360pt card.** `.overlay` sizes
itself to the view it is attached to, and the widget is a card in the hero row —
so `maxWidth: 420` inside it could never dock to the window edge. The panel's
state moved up to the dashboard.

### New web defects

| # | Defect |
|---|---|
| 50 | **`Suggestions.tsx`'s query is invalid SQL and the strip has never rendered.** It ends `(SELECT COUNT(*) FROM credit_card_details …) AS creditCards,` — trailing comma, no `FROM`. The query throws, `usage` stays `{}`, and `pickSuggestions` returns nothing |
...
## Tranche 8 — field-level encryption and the security panel (2026-08-29)

The last two Large gaps outside the two blocked ones. Both platforms now do the
whole of it: passphrase setup, unlock by passphrase or recovery code, the
recovery code, Lock, structural and content support grants with revoke, and
note encryption on every write path web has.

### The scheme, pinned

Read out of `packages/core/crypto/src/index.ts`, not guessed:

| Piece | Value |
|---|---|
| Envelope | `v1.<base64(iv)>.<base64(ciphertext‖tag)>` — padded base64, split on `.`, exactly 3 parts |
| Cipher | AES-256-GCM, 12-byte IV, 128-bit tag |
| KDF | PBKDF2-HMAC-SHA-256, **210 000** iterations, 16-byte per-user salt, 256-bit output |
| DEK | random 256-bit, wrapped by KEK(passphrase) **and** KEK(recovery code) under the same salt |
| Recovery code | 20 random bytes → `alphabet[b % 31]` over `ABCDEFGHJKMNPQRSTUVWXYZ23456789`, dashes every 4. **The dashes are part of the KDF input** |
| Support grant | DEK re-wrapped RSA-OAEP-3072/SHA-256; payload canonical-JSON, signed ECDSA P-256, raw r‖s; 2h TTL |

`security.json` — **53 vectors generated by running web's own WebCrypto**, not
transcribed. It pins the envelope format, the recovery-code encoding, the
canonical grant JSON, `deriveKekHex` at the real iteration count including a
non-ASCII passphrase, decryption of **browser-written ciphertext**, and a
bit-flipped ciphertext that GCM must reject. That last set is the point: they
are the only proof that an envelope written in a browser opens on a phone.

**PBKDF2 is hand-rolled over HMAC-SHA256 on both platforms** (RFC 8018 §5.2).
Android's stock `PBKDF2WithHmacSHA256` converts the passphrase with PKCS#5's
8-bit rule rather than UTF-8, so any non-ASCII passphrase would derive a
different key from the browser's and the user would be told their correct
passphrase is wrong. The non-ASCII vector is what pins that, and the multi-block
vectors (20, 32, 64 and 100-byte outputs) exercise the block index and offset
arithmetic that a single 32-byte output never reaches.

### Two divergences from web, both deliberate

**Web never shows anybody the recovery code.** `setupEncryption()` sets
`hasKeys = true` and calls `notify()` *before* returning the code, so the panel
re-renders into the unlocked box and unmounts the setup box before its
`setRecovery(code)` can paint. The one-time code — the only way back in after a
forgotten passphrase — is displayed to no one. Both ports check the recovery
branch ahead of the status machine and hold until it is acknowledged.
Reproducing web here would ship a feature whose failure mode is permanently
unreadable data.

**The unlock is persisted in the platform secure store.** Web's DEK is
memory-only. This is a real change to the threat model and is written up as a
decision record in `ABSENT-BY-DECISION.md` — **it needs sign-off**, and the two
better alternatives (device-authentication-gated unwrap, or a timed lock) are
costed there.

### Caught in review, before commit

**Setup could have permanently destroyed data.** `refreshKeyState()` read
`user_keys` from the local PowerSync copy; on a fresh install, before that table
syncs, `hasKeys` is false, the panel offers setup, a second key set is created,
and the upsert overwrites the row — **every note encrypted under the first DEK
unreadable on every device, forever**. Web has the same shape but loads over the
network; a phone syncs lazily in the background, so the window is far wider.

Now defended twice. `refreshKeyState()` resolves in three tiers: a local row is
positive proof; **absence is believed only when PostgREST confirms it**; anything
else leaves the state nil, which renders as "Checking…" and never as the setup
form. And `setupEncryption` does `insert`, not `upsert` — `user_keys.user_id` is
the primary key, so **Postgres itself refuses**, and the refusal is reported as
"Unlock it instead — setting it up again would make your existing notes
unreadable." The display gate depends on the client remembering to check; the
insert does not.

**A locked session put the raw envelope in an editable field.** Touching it saved
the mangled string back — and because it still began `v1.`, `encryptForWrite`
passed it through unchanged and the note was destroyed. Rendering the envelope is
web parity; editing it is not. Read-only when locked, with a hint.

Also fixed: `AlgorithmParameters.getInstance("EC")` (patchy Android provider
history) replaced by reading the curve off a generated keypair; crypto-provider
failures no longer swallowed into "Couldn't create grant", and a PBKDF2 failure
no longer reports "Wrong passphrase."; an unguarded `refreshKeyState()` that
could crash the process on a cold start; Keystore/Keychain I/O moved off the main
thread; and `dek()` now returns a copy, so a `lock()` racing an in-flight encrypt
can no longer encrypt under a zeroed key.

### New web defects

| # | Defect |
|---|---|
| 54 | **The recovery code is shown to nobody.** `setupEncryption()` notifies before returning it, so the setup box unmounts before it can paint. The plan's own closing line says the UI "must make the recovery code impossible to skip at setup" |
| 55 | **Only `note` is ever encrypted.** The plan lists `transactions.description`, `transaction_items.description`, `accounts.name`, `card_last4`, `labels.name`, goal and subscription names. None of them are |
| 56 | **Envelopes render as ciphertext in six read sites** — reflect, search, the insight stack, the budget breakdown, CSV export and CSV import all use `description \|\| note` as a fallback label — and `t.note LIKE ?` cannot match an encrypted note. `fields.ts` exports a `useDecrypted` hook that is called from nowhere, and a `•••••` masking helper that is never used. Encrypted notes are unsearchable and export as ciphertext on all three clients |
| 57 | The edit form shows a raw `v1.…` envelope in a locked session rather than the mask that already exists in `fields.ts` |

### Not resolved — needs a decision

**No support public key exists anywhere in this repo.** Web reads
`NEXT_PUBLIC_SUPPORT_PUBLIC_JWK` from the deploy environment; `support-key/`
holds only Shamir shares. An optional `supportPublicJwk` is plumbed through both
mobile configs with an empty default, so a content grant refuses with web's own
"Support access is not configured for this deployment." Structural grants work
regardless. **The RSA-OAEP wrap path has never been exercised against a real key
on any platform, web included** — the pure parts (DER→raw, PKCS#1 DER) are
vector-pinned, the provider calls around them are not. Consider gating the two
grant chips behind a flag until a key exists.
