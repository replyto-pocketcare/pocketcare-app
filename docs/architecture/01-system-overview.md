...
1. **Money is integer minor units**, never floats (`Money` type in `@sanvya/money`).
...
flowchart TB
    subgraph Device["User Device (browser / installed PWA)"]
        ...
        Repos["Repositories<br/>@sanvya/data"]
...
Sanvya deliberately uses **two distinct paths** to the backend:
...
flowchart LR
    ...
    D -->|"Privileged / online-only"| R["supabase.schema('sanvya').rpc()<br/>or Edge Function"] --> PG
    PG -->|"sync stream"| L2["Local SQLite (all devices)"]
```

> **Gotcha (historical bug):** every table and RPC lives in the **`sanvya`** schema, not `public`. Direct calls must be schema-qualified (`supabase.schema('sanvya').rpc(...)`) or PostgREST returns 404. See [04 — Security & Privacy](04-security-and-privacy.md#account-deletion).
