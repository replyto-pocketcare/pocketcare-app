...
>
> **Progress:** Phase A done. Phase B token generator done (`tools/parity/generate-tokens.mjs`, source of truth `apps/web/app/globals.css`) — caught iOS `Theme.swift` was missing 6 tokens including `warning`. First screen fully through the pipeline (2026-08-05): `docs/mobile/screen-specs/dashboard.md` written from `apps/web/app/page.tsx` (source-translation, not screenshots — the method this plan settled on), then built for real on both platforms. Along the way, discovered Android had no working app at all beyond the theme gap — no `Application` class (manifest pointed at a class that never existed, Koin was never started), no `Prefs`, no `SanvyaNavHost` — all three built for real, not stubbed. iOS's existing Dashboard hero turned out to be an invented design (flat accent card, fake Assets/Liabilities split) rather than a port of web's actual gradient/sparkline hero — replaced to match the spec. Tile catalog (12 tiles, `apps/web/src/dashboard/tiles.tsx`) explicitly deferred as its own tracked task, not faked. **Not yet done:** none of this has been verified against a real Gradle/Xcode build (no toolchain in this sandbox) — flagged explicitly in `docs/mobile/TODO.md`, not claimed. Every other screen still needs its own source spec + build pass.
