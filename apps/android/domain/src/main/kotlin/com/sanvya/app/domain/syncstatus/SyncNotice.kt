 ...
 *
 * Mirrors apps/ios/Domain/Sources/Domain/SyncNotice.swift.
 */
 ...
 * whether their error is going to be swallowed. Keep the two in step -- adding
 * a token here without adding it to `sync.ts` makes the ports disagree about
 * when to stay quiet.
 */
private val TRANSIENT_TOKENS = listOf(
    "websocket",
    "network",
    "failed to fetch",
    "load failed",
 ...
 */
fun isTransientSyncError(error: String?): Boolean {
    if (error.isNullOrBlank()) return false
    val lowered = error.lowercase()
    return TRANSIENT_TOKENS.any { lowered.contains(it) }
}
 ...
 */
fun syncNotice(online: Boolean, error: String?): SyncNotice = when {
    !online -> SyncNotice.OFFLINE
    error.isNullOrBlank() -> SyncNotice.NONE
    isTransientSyncError(error) -> SyncNotice.NONE
    else -> SyncNotice.TROUBLE
}
