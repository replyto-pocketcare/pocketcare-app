...
import { insertRow, softDelete, nowIso } from "../write";
import { LockIcon } from "../ui/icons";
import { useConfirm } from "../ui/Confirm";
import { buildFinancialSummary, summaryForPrompt } from "./summary";
import { parseAssistantMessage, AssistantUiBlock, Markdown } from "./richMessage";
import { ASSISTANT_TOOLS, executeTool, describeToolCall, needsConfirm, isValidToolInput, loadMemory } from "./tools";
import { MicButton } from "./MicButton";
import { ArrowUpIcon } from "../ui/icons";
import { buyCredits } from "../billing";
import { useEntitlement } from "../entitlement";
import { CREDIT_PACKS } from "../billing/plans";
...
/**
 * The assistant, as a component rather than a page.
 *
 * Extracted so the dashboard's Ask Sanvya panel and the /assistant route run
 * the SAME chat -- threads, tools, quota, confirmations and all. The panel
 * used to hand off by navigating to /assistant with the question in the URL,
 * which kept one implementation but threw the user off the dashboard to ask a
 * question about it. Embedding is the fix; a second, thinner chat inside the
 * panel would have been the wrong one, because it would drift from this the
 * moment either changed.
 *
 * `embedded` affects framing only: the standalone route sizes itself to the
 * visual viewport (so the on-screen keyboard shrinks the frame instead of
 * covering the composer), while an embedded instance fills the box its host
 * gives it.
 */
export function AssistantChat({ embedded = false, initialPrompt }: {
  embedded?: boolean;
  ...
  useEffect(() => {
    // Embedded, the host panel owns the box; measuring the visual viewport
    // here would size this to the whole screen and lock the page behind it.
    if (embedded || view !== "chat") return;
  ...
  useEffect(() => {
    ...
    const q = (initialPrompt ?? new URLSearchParams(window.location.search).get("q") ?? "").trim();
    if (!q) return;
    deepLinked.current = true;
    // Drop the param so a refresh (or back) doesn't silently re-ask and
    // re-spend the user's quota.
    if (!initialPrompt) window.history.replaceState(null, "", window.location.pathname);
    setView("chat");
    void sendText(q);
