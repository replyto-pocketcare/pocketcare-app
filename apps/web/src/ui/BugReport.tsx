...
export function BugReportModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function reset() {
    setSeverity("medium"); setArea(""); setTitle(""); setDescription(""); setDone(false); setErr(null);
  }
  ...
  async function submit() {
    ...
    try {
      await insertRow("bug_reports", {
        severity, area: area || null, title: title.trim() || null, description: description.trim(), status: "open",
    ...
    <Modal open={open} onClose={() => { onClose(); setTimeout(reset, 200); }}>
      {done ? (
        <div style={{ display: "grid", gap: 10, textAlign: "center" }}>
          ...
          <h2 style={{ margin: 0 }}>Thanks for the report!</h2>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          <div>
            <h2 style={{ margin: 0 }}>Report a bug or suggestion</h2>
            <p className="muted" style={{ fontSize: 12, margin: "4px 0 0" }}>Pick a couple of options and tell us what happened — we capture the rest automatically.</p>
          </div>
          ...
          <textarea className="input" rows={4} placeholder="What went wrong? What did you expect to happen?" value={description} onChange={(e) => setDescription(e.target.value)} style={{ resize: "vertical" }} />
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            ...
            <button className="btn" onClick={submit} disabled={busy || !description.trim()}>{busy ? "Sending…" : "Send report"}</button>
          </div>
        </div>
      )}
    </Modal>
