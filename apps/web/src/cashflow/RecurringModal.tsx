...
const DIR_LABEL: Record<RecurringDirection, string> = { income: "recurring income", payment: "recurring payment", saving: "recurring saving" };
...
export function RecurringModal({ direction, base, edit, prefill, onClose }: {
  ...
  const accountLabel = direction === "income" ? "Deposit into" : isSaving ? "Fund from" : "Pay from";
  ...
  return (
    <Modal open onClose={() => onClose(false)}>
      <div style={{ display: "grid", gap: 12 }}>
        <h2 style={{ margin: 0, textTransform: "capitalize" }}>{edit ? "Edit" : "Add"} {DIR_LABEL[direction]}</h2>

        <FloatingInput label="Name" value={name} onChange={setName} />
        <FloatingInput label={`Amount (${base})`} group currency={base} value={amount} onChange={setAmount} />
        ...
        {isSaving && (
          <label className="muted" style={{ fontSize: 12, display: "grid", gap: 4 }}>Into investment account
            {investAccounts.length === 0
              ? <span style={{ color: "var(--negative)" }}>No investment account yet — add one on the Investments page first.</span>
        ...
        {isPayment && (
          <label className="muted" style={{ fontSize: 12, display: "grid", gap: 4 }}>Category (optional)
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap" }}>
          <div style={{ display: "grid", gap: 4, flex: 1, minWidth: 180 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {FREQS.map((f) => <button key={f} className="chip" data-active={f === freq} style={{ textTransform: "capitalize" }} onClick={() => setFreq(f)}>{f}</button>)}
            </div>
          </div>
          <label className="muted" style={{ fontSize: 12, display: "grid", gap: 4, width: 160 }}>First due
        ...
        <label style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 13, cursor: "pointer" }}>
          ...
          <span>Post automatically on the due date<br /><span className="muted" style={{ fontSize: 12 }}>Off = we’ll ask you to confirm each time it’s due.</span></span>
        </label>

        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 2 }}>
          <button className="btn ghost" onClick={() => onClose(false)} disabled={saving}>Cancel</button>
          <button className="btn" onClick={submit} disabled={!canSave || saving}>{saving ? "Saving…" : edit ? "Save" : "Add"}</button>
        </div>
      </div>
    </Modal>
  );
