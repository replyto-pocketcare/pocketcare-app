...
export interface Template {
  ...
  split_group_id: string | null; split_mode: string | null; group_id: string | null;
}

/** Alphabetical — manual ordering was removed, see templates/write.ts. */
export function useTemplates(): Template[] {
  const { data = [] } = useQuery<Template>(
    "SELECT id, name, type, amount, currency, account_id, to_account_id, category_id, description, note, payment_method, labels, split_group_id, split_mode, group_id FROM transaction_templates WHERE deleted_at IS NULL ORDER BY name COLLATE NOCASE",
  );
  return data;
}
