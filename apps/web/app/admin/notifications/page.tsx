...
import { getNotificationGroups, sendBroadcastPush, createNotificationGroup } from "../../../src/admin-actions";
...
export default function AdminNotifications() {
  ...
  const buttonStyle = (active: boolean) => ({
    ...
    background: active ? "#555" : "#cc6644", color: "#fff",
    fontWeight: "bold", cursor: active ? "not-allowed" : "pointer",
    width: "100%"
  });

  const StatusBox = ({ s }: { s: { type: "success" | "error"; msg: string } | null }) => s ? (
    <div style={{ padding: 12, borderRadius: 8, marginBottom: 16, background: s.type === "error" ? "#4a1212" : "#124a1f", border: `1px solid ${s.type === "error" ? "#f00" : "#0f0"}` }}>
      {s.msg}
    </div>
  ) : null;
      ...
      {loading ? (
        ...
        <>
          ...
          <form onSubmit={handleCreateGroupSubmit} style={{ background: "#222", borderRadius: 12, border: "1px solid #333", padding: 30 }}>
            <h3 style={{ margin: "0 0 20px" }}>Create Notification Group</h3>
            ...
            <input 
              ...
              placeholder="e.g. Users testing unreleased features"
              style={inputStyle}
            />

            <StatusBox s={groupStatus} />

            <button type="submit" disabled={creatingGroup || !newGroupName} style={buttonStyle(creatingGroup)}>
