...
import { useGroups, useContacts } from "../../src/splits/hooks";
import { createGroup, addContact } from "../../src/splits/write";
    ...
    <div style={{ display: "grid", gap: 20 }} className="fade-up">
      ...
      {groups.length === 0 ? (
        <div className="card" style={{ padding: 32, textAlign: "center", display: "grid", gap: 10, justifyItems: "center" }}>
          ...
          <p className="muted" style={{ margin: 0, maxWidth: 380 }}>Create a trip (e.g. “Goa”) or a household group, add friends, then split expenses into it.</p>
          ...
          {groups.map((g) => (
            <Link key={g.id} href={`/groups/${g.id}`} className="card" style={{ padding: 16, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
              <div style={{ minWidth: 0 }}>
                ...
                <span className="muted" style={{ fontSize: 12 }}> · {g.kind}</span>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input className="input" placeholder="Add a person…" value={newContact}
              ...
              onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); void addNewContact(); } }} />
            <button type="button" className="chip" onClick={() => void addNewContact()} disabled={!newContact.trim()}>Add</button>
          </div>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 4 }}>
            ...
            <button className="btn" onClick={() => void create()} disabled={busy || !name.trim() || members.length === 0}>{busy ? "Creating…" : "Create"}</button>
          </div>
        </div>
      </Modal>
    </div>
