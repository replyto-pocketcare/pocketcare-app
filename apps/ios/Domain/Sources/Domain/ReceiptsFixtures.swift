...
// Ported from packages/core/receipts/src/fixtures.ts (P1.5b). Mirrors
// apps/android/domain/.../receipts/ReceiptsFixtures.kt (P1.5a). Only
// name/text/currency are reproduced -- the TS file's `expect` blocks aren't
// needed here since correctness is judged against the actual exported
// golden vectors, not a second copy of the expectation. Transcribed
// character-for-character from the TS source's template strings, including
// exact whitespace/newlines, since parseReceiptText's vectors were
// exported by running the REAL text through the REAL parser.

public struct ReceiptFixture: Sendable {
    public let name: String
    public let text: String
    public let currency: String
}

public let RECEIPT_FIXTURES: [ReceiptFixture] = [
    ReceiptFixture(
