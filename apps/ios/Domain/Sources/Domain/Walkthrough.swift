...
/// not set anything up.
///
/// Mirrors apps/android/domain/.../onboarding/Walkthrough.kt.
public func shouldShowWalkthrough(
    done: Bool,
    skipped: Bool,
    syncPending: Bool,
    accountCountLoaded: Bool,
    realAccountCount: Int,
    signedIn: Bool
) -> Bool {
    !done && !skipped && !syncPending && accountCountLoaded && realAccountCount == 0 && signedIn
}
...
public struct WalkthroughProgress: Equatable, Sendable {
    public let step: Int
    public let of: Int
}

public func walkthroughProgress(_ step: Int) -> WalkthroughProgress {
    step <= 4 ? WalkthroughProgress(step: step, of: 4) : WalkthroughProgress(step: step - 4, of: 3)
}
