...
- **Multi-device identity:** `src/powersync.ts` must re-key the local DB on `onAuthStateChange` when the user id changes (sign-in on a new device switches from the boot-time anonymous guest to the real account). Without it, PowerSync keeps syncing the guest and nothing crosses devices. Guest→register via `updateUser` keeps the same UID (no clear).
