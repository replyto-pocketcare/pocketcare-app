# PROJECT_REFERENCE.md — PocketCare (LLM boot file)
...
1. Money = integer minor units via `@pocketcare/money`. Never floats.
2. Balances derive from the append-only ledger; corrections are compensating entries, never mutation.
3. Everything server-side lives in the **`pocketcare`** schema — schema-qualify every direct call/RPC.
...
- Some FKs are deliberately ABSENT on synced child rows (e.g. `transaction_templates.group_id`, loan→account link): a child can upload before its parent → 23503 → quarantine. Client enforces; `pocketcare.audit_*()` fns observe.
...
- `TransactionTile` is the ONLY transaction row component. One allocation implementation (`@pocketcare/receipts` `splitByWeights` — `splits/math.ts` re-exports it).
...
- Failure handling: `@pocketcare/sync-policy` classifies (default transient; 401/408/429 transient; SQLSTATE beats HTTP status); permanent → 3 attempts → quarantine to local-only `failed_writes` (write-then-delete order); surfaced by ProblemsPanel (named rows, export-before-discard ALWAYS, sequential direct retry). No `ON CONFLICT` on PowerSync views.
...
## Testing
`pnpm test:core` — 290 tests, Node `node:test` + type-stripping (sandbox: `node --test --experimental-strip-types packages/core/*/src/*.test.ts`). Web: `pnpm --filter @pocketcare/web typecheck`. Full strategy + ~100-case catalog: `docs/plans/full-test-plan.md`. Money/domain tests go in `packages/core` (the spec), never duplicated per app.
