...
create table if not exists pocketcare.settlements (
...
create index if not exists settlements_contact_idx on pocketcare.settlements(contact_id);
create index if not exists settlements_user_idx on pocketcare.settlements(user_id);

alter table pocketcare.settlements enable row level security;
create policy settlements_owner on pocketcare.settlements using (user_id = auth.uid()) with check (user_id = auth.uid());
grant all on table pocketcare.settlements to anon, authenticated, service_role;
